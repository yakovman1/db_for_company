...
def _ensure_access(user: UserContext, project_id: uuid.UUID) -> None:
...
async def init_upload(user: UserContext, payload: InitUploadRequest, session) -> InitUploadResponse:
    _ensure_access(user, payload.project_id)

    family_id = uuid.uuid4()
    object_key = _build_object_key(payload.project_id, family_id, payload.sha256, payload.original_filename)
...
async def save_metadata(user: UserContext, family_id: uuid.UUID, metadata: FamilyMetadataPayload, session) -> Family:
    family = await repo.get_family(session, family_id)
    if not family:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Family not found")

    _ensure_access(user, family.project_id)
...
async def complete_upload(user: UserContext, family_id: uuid.UUID, payload: CompleteUploadRequest, session) -> Family:
    family = await repo.get_family(session, family_id)
    if not family:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Family not found")
    _ensure_access(user, family.project_id)
...
async def get_family(user: UserContext, family_id: uuid.UUID, session) -> Family:
    family = await repo.get_family(session, family_id)
    if not family:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Family not found")
    _ensure_access(user, family.project_id)
...
async def list_families(user: UserContext, project_id: uuid.UUID, limit: int, offset: int, session):
    _ensure_access(user, project_id)
    families = await repo.list_families_by_project(session, project_id=project_id, limit=limit, offset=offset)
    total = await repo.count_families_by_project(session, project_id=project_id)
    return families, total


async def get_download_url(user: UserContext, family_id: uuid.UUID, session) -> str:
    family = await repo.get_family(session, family_id)
    if not family:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Family not found")
    _ensure_access(user, family.project_id)
